package com.sangsang.jdncsharding.config;
...
@Configuration
@MapperScan(basePackages = "com.sangsang.jdncsharding.mapper", sqlSessionTemplateRef  = "test1SqlSessionTemplate")
...
//        return DataSourceBuilder.create().build();
    ...
    public ShardingRule shardingRule(DataSourceRule dataSourceRule){
        //具体分库分表策略
        TableRule orderTableRule = TableRule.builder("t_order")//暴露的虚拟表
                .actualTables(Arrays.asList("t_order_0", "t_order_1"))//真实需要进行分库分表的表
                .tableShardingStrategy(new TableShardingStrategy("order_id", new ModuloTableShardingAlgorithm()))//根据order_id 进行分表策略
        ...
        return ShardingRule.builder()
                ...
                .databaseShardingStrategy(new DatabaseShardingStrategy("user_id", new ModuloDatabaseShardingAlgorithm()))//以user_id 的值进行分库
                .tableShardingStrategy(new TableShardingStrategy("order_id", new ModuloTableShardingAlgorithm()))//以order_id 的值进行分表
                .build();
    }
    ...
    public SqlSessionFactory testSqlSessionFactory(@Qualifier("dataSource") DataSource dataSource) throws Exception {
        ...
        bean.setMapperLocations(new PathMatchingResourcePatternResolver().getResources("classpath:com/sangsang/jdncsharding/mapper/*.xml"));
